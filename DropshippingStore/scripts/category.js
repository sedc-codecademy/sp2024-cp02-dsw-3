...
const addToCartService = {
    ...
    cartEvent: function (img) {
        let itemsInStorage = storage.getFromLocalStorage("cart-items") || [];
        
        if (!itemsInStorage.some(item => item.id === img.id)) {
            itemsInStorage.push(img);
            storage.setToLocalStorage("cart-items", itemsInStorage);
            itemsInCart.displayItems();
        }
    }
};
...
const itemsInCart = {
    ...
    displayItems: function(){
        this.cart.innerText = `${storage.itemsInCart("cart-items")}`
    }
}
